...
const getblog = (req,res)=>{
    const token=req.cookies.jwt;
        jwt.verify(token,process.env.SECRET,async(err,decodedToken)=>{
                ...
                User.findById(decodedToken.id).then((result)=>{res.render('view-blogs',{title:'Blogs', data:result.blog})}).catch((err)=>{console.log(err);})
        });
...
const deleteid = (req,res)=>{
    ...
    jwt.verify(token,process.env.SECRET,async(err,decodedToken)=>{
        User.findByIdAndUpdate(
            decodedToken.id, { $pull:  { blog: {_id:id} }  },
            function(err, node) {
...
module.exports={
    home,postblog,getblog,about,createblog,getid,deleteid,Err,signup_get,login_get,signup_post,login_post,logout_get
}